  ...
  if (state.conversationId) {
    try {
      ...
      for (const t of r.turns) {
        if (t.turn_role === 'user') {
          addMessage('user', { text: t.content });
          state.history.push({ role: 'user', content: t.content });
        } else {
          addMessage('bot', { prefix: t.content, sql: t.sql });
          state.history.push({
            role: 'assistant',
            content: t.sql ? `${t.content}\n[SQL] ${t.sql}` : (t.content || ''),
          });
        }
        state.transcript.push({
          role: t.turn_role === 'user' ? 'user' : 'assistant',
          text: t.content, sql: t.sql,
        });
      }
...
async function loadHealth() {
  try {
    ...
    else if (h.services?.anthropic) { label = 'LLM: Claude'; cls = 'bg-emerald-900/40 text-emerald-300'; }
...
function showView(v) {
  ...
  const titles = {
    chat: ['Chat', 'Ask in English or Kannada. Voice is supported.'],
    hotspots: ['Hotspots', 'Districts by crime volume, last 180 days.'],
    trends: ['Trends', 'Monthly volume by category.'],
    network: ['Criminal Network', 'Co-offenders sharing 2+ crimes.'],
    insights: ['Insights', 'Socio-demographic profile & repeat-offender behaviour.'],
    predict: ['Early Warnings', '30-day vs prior 30-day district × category deltas.'],
    audit: ['Audit Log', 'Every query, every user, forever traceable.'],
  };
  $('#viewTitle').textContent = titles[v][0];
  $('#viewSubtitle').textContent = titles[v][1];
...
function renderTable(columns, rows) {
  ...
  const tbody = el('tbody', {}, rows.slice(0, 25).map(r =>
    el('tr', {}, columns.map(c => el('td', {}, String(r[c] ?? ''))))
  ));
...
function renderExplain(r, notes) {
  ...
  div.appendChild(kv('Language detected', r.language));
  div.appendChild(kv('LLM explanation', r.explanation_en));
  if (r.explanation_kn) div.appendChild(kv('ವಿವರಣೆ', r.explanation_kn));
  if (r.sql) {
    div.appendChild(el('div', {}, [
      el('div', { class: 'text-[11px] uppercase tracking-wider text-slate-500 mb-1' }, 'SQL executed'),
      el('pre', { class: 'sql' }, r.sql),
    ]));
  }
  if (notes?.length) {
    div.appendChild(kv('Role policy notes', notes.join('; ')));
  }
  const providerLabel = { quickml: 'Catalyst QuickML', claude: 'Claude',
                          fallback: 'keyword fallback (offline)' };
  div.appendChild(kv('Provider', providerLabel[r.provider] || r.provider));
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  $('#langSel').addEventListener('change', (e) => {
    state.lang = e.target.value; persistSession();
  });
