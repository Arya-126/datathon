...
# ============================================================
def smartbrowz_pdf(html: str, capp=None) -> bytes | None:
    """Render HTML → PDF bytes via Catalyst SmartBrowz, or None if the
    service isn't reachable (caller falls back to client-side jsPDF)."""
    if capp is None:
        return None
    try:
