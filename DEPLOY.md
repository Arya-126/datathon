...
### 2.1 · `ANTHROPIC_API_KEY` — Claude fallback for NL→SQL

1. https://console.anthropic.com → **API Keys** → *Create Key* → copy.
2. Paste into `ANTHROPIC_API_KEY`. Leave `ANTHROPIC_MODEL=claude-fable-5`.
3. Production best practice: create a **Catalyst Connection** for
   Anthropic (Console → Connections → *Create* → Custom Service → OAuth 2
   / API Key) so the credential is Catalyst-managed rather than baked into
   env; expose it to the AppSail via the same env var name.
...
create-table API), so we do it once, then let the app write rows.

**Option A — console upload (recommended, 15 min):**
...
2. Console → project → **Data Store** → *Import Data* → upload each CSV in
   the order listed in `datastore_export/SCHEMA.md`
   (`State` → `District` → … → `PersonAlias` → app tables). Catalyst infers
   column types from the header + first rows; if any column type looks
   wrong (e.g. `ID`s should be *Bigint*), edit before confirming import.
...
# The frontend's LLM badge upgrades to "QuickML" or "Claude" the
...
## What can't be automated from a Claude session
